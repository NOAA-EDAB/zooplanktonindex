...
We compared a preliminary annual scale VAST *Calanus finmarchicus* model output (no covariates or bias correction) with the annual anomalies currently included in the R `ecodata` package. Estimation methods are completely different and spatial strata are slightly different, but the timing of high and low anomalies is well-aligned between the two methods for the region mainly occupied by both Atlantic herring and *Calanus finmarchicus*, Georges Bank (GB) and the Gulf of Maine (GOM). 
