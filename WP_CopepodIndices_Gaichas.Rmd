...
Model selection was done in two stages, the first looking at spatial, spatio-temporal random effects and the second looking at covariates, following @ng_predator_2021 and @gaichas_assessing_2023.  The model selection script is available at 
...
We compared a preliminary annual scale VAST *Calanus finmarchicus* model output (no covariates or bias correction) with the annual anomalies currently included in the R `ecodata` package ([overview](https://noaa-edab.github.io/catalog/zoo_abundance_anom.html), [methods](https://noaa-edab.github.io/tech-doc/zoo_abundance_anom.html#data-analysis-11)). Estimation methods are completely different and spatial strata are slightly different, but the timing of high and low anomalies is well-aligned between the two methods for the region mainly occupied by both Atlantic herring and *Calanus finmarchicus*, Georges Bank (GB) and the Gulf of Maine (GOM). 
...
#url <- "https://raw.githubusercontent.com/NOAA-EDAB/presentations/docs/EDAB_images/EPU_Designations_Map.jpg"

knitr::include_graphics("/Users/sarah.gaichas/Documents/0_Data/Presentations/presentations/docs/EDAB_images/EPU_Designations_Map.jpg")
```


## Model selection Stage 1 results
...
## Model selection stage 2 results
...
Full results from the best fitting models are posted to google drive [here](https://drive.google.com/drive/folders/1ckEX3FEzxFFE0y49l7uRYciX0HFv-Utq?usp=sharing)
...
### Spring (January-June) large copepods
...
### Fall (July-December) small copepods
...
### September-February small copepods in herring larval area
...
```{r smcopelarvfalldens}
knitr::include_graphics(here::here("pyindex/smallcopeALL_sepfeb_yrshift_500_larvarea_biascorrect/ln_density-predicted.png"))
```

### Summary figure for ToR 1

Indices sent to the WHAM model

```{r summfig1, fig.asp=1}

lg <- plot_zooindices(splitoutput = splitoutput |> dplyr::filter(Season == "spring"), 
             plotdata = "lgcopeALL", 
             plotregions = c("her_sp"), #"AllEPU", "SS"
             plottitle = "Spring Large copeopods") 

sm <- plot_zooindices(splitoutput = splitoutput |> dplyr::filter(Season == "fall"), 
             plotdata = "smallcopeALL", 
             plotregions = c("her_fa"), #"AllEPU", "SS"
             plottitle = "Fall Small copeopods") 
