...
##################################################################################
# zoopvolume model inputs

zoopvol_stn_ann <- herringfood_stn %>%
  #ungroup() %>%
...
zoopvol_stn_fall <- herringfood_stn %>%
  #ungroup() %>%
...
zoopvol_stn_spring <- herringfood_stn %>%
  #ungroup() %>%
...
mod.season <- c("zoopvol_fall_500", "zoopvol_spring_500", "zoopvol_ann_500") #includes n knots

mod.dat <- list(zoopvol_stn_fall, zoopvol_stn_spring, zoopvol_stn_ann)

names(mod.dat) <- mod.season

mod.obsmod <- list(ObsModel2, ObsModel2, ObsModel2)
