...
herringfood_stn_fall <- herringfood_stn %>%
  #ungroup() %>%
  filter(season_4 == "fall", # Annual model for MRIP index
         year > 1976) %>%
...
herringfood_stn_spring <- herringfood_stn %>%
  #ungroup() %>%
  filter(season_4 == "spring", # Annual model for MRIP index
         year > 1976) %>%
  mutate(AreaSwept_km2 = 1, #Elizabeth's code
         #declon = -declon already done before neamap merge
         Vessel = 1 #as.numeric(as.factor(vessel))-1
...
herringfood_stn_summer <- herringfood_stn %>%
  #ungroup() %>%
  filter(season_4 == "summer", # Annual model for MRIP index
         year > 1976) %>%
  mutate(AreaSwept_km2 = 1, #Elizabeth's code
         #declon = -declon already done before neamap merge
         Vessel = 1 #as.numeric(as.factor(vessel))-1
...
herringfood_stn_winter <- herringfood_stn %>%
  #ungroup() %>%
  filter(season_4 == "winter", # Annual model for MRIP index
         year > 1976) %>%
  mutate(AreaSwept_km2 = 1, #Elizabeth's code
         #declon = -declon already done before neamap merge
         Vessel = 1 #as.numeric(as.factor(vessel))-1
...
season <- c("fall_4season_500_test")
...
season <- c("spring_4season_500_test")
...
season <- c("winter_4season_500_test")
...
fit <- try(fit_model( settings = settings,  
                      #extrapolation_list = New_Extrapolation_List,
                      Lat_i = herringfood_stn_winter[,'Lat'], 
                      Lon_i = herringfood_stn_winter[,'Lon'], 
                      t_i = herringfood_stn_winter[,'Year'], 
                      b_i = as_units(herringfood_stn_winter[,'Catch_g'], 'g'), 
                      a_i = rep(1, nrow(herringfood_stn_winter)),
                      v_i = herringfood_stn_winter$Vessel,
                      #Q_ik = as.matrix(herringfood_stn_winter[,c("meanbenthivorelen", 
                      #                                        "nbenthivoresp", 
                      #                                        "btfill")]),
                      # Use_REML = TRUE,
                      working_dir = paste0(working_dir, "/"))
...
season <- c("summer_4season_500_test")
...
fit <- try(fit_model( settings = settings,  
                      #extrapolation_list = New_Extrapolation_List,
                      Lat_i = herringfood_stn_summer[,'Lat'], 
                      Lon_i = herringfood_stn_summer[,'Lon'], 
                      t_i = herringfood_stn_summer[,'Year'], 
                      b_i = as_units(herringfood_stn_summer[,'Catch_g'], 'g'), 
                      a_i = rep(1, nrow(herringfood_stn_summer)),
                      v_i = herringfood_stn_summer$Vessel,
                      #Q_ik = as.matrix(herringfood_stn_summer[,c("meanbenthivorelen", 
                      #                                        "nbenthivoresp", 
                      #                                        "btfill")]),
                      # Use_REML = TRUE,
                      working_dir = paste0(working_dir, "/"))
