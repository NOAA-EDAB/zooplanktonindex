...
##################################################################################
# euphausiid model inputs

euph_stn_ann <- herringfood_stn %>%
  #ungroup() %>%
  ...
  dplyr::select(Catch_g = euph_100m3, #use megabenwt for individuals input in example
                Year = year,
                Vessel,
                AreaSwept_km2,
                Lat = lat,
...
euph_stn_spring <- herringfood_stn %>%
  #ungroup() %>%
