...
herringfood_stn_ann <- herringfood_stn %>%
  #ungroup() %>%
...
fit <- fit_model( settings = settings,  
                  #extrapolation_list = New_Extrapolation_List,
                  Lat_i = herringfood_stn_ann[,'Lat'], 
                  Lon_i = herringfood_stn_ann[,'Lon'], 
                  t_i = herringfood_stn_ann[,'Year'], 
                  b_i = as_units(herringfood_stn_ann[,'Catch_g'], 'g'), 
                  a_i = rep(1, nrow(herringfood_stn_ann)),
                  v_i = herringfood_stn_ann$Vessel,
                  #Q_ik = as.matrix(herringfood_stn_ann[,c("meanbenthivorelen", 
                  #                                        "nbenthivoresp", 
                  #                                        "btfill")]),
                  # Use_REML = TRUE,
                  working_dir = paste0(working_dir, "/"))
