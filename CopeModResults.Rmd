...
Model selection looked at inclusion of different spatial and spatio-temporal random effects. Testing always included all of them. See [here](https://noaa-edab.github.io/zooplanktonindex/CopeModSelection.html). 
...
Catchability covariates were difficult to envision for a zooplankton survey, but we tried Day of Year to compensate for changes in standing stock over time for a high turnover species. This covariate ("doy" in model names) often resulted in models with lower AIC but poor convergence properties. Therefore we only used this covariate when AIC was lower *and* the model met convergence critiera. 
...
```{r, results = "asis"}
for(d.name in moddirs[str_detect(moddirs, "calfin")][c(2,4)]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "calfin")]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "lgcopeALL")]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "smallcopeALL")]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "smallcopeSOE")]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "zoopvol")]){
...
```{r, results = "asis"}
for(d.name in moddirs_best[str_detect(moddirs_best, "euph")]){
