...
First make the 70th percentile+ points into an `sf` object, then intersect that object with the built in `FishStatsUtils::northwest_atlantic_grid`, then define new strata based on that intersection, and call them as strata.limits when running the small copepods model.
...
D70pct_nwa <- sf::st_intersection(nwagrid_sf,D70pct_ls) %>%
...
# intersect, rearrange in same format as nwatl grid, and save
D75pct_nwa <- sf::st_intersection(nwagrid_sf,D75pct_ls) %>%
  dplyr::mutate(Lon = as.numeric(sf::st_coordinates(.)[,1]),
                Lat = as.numeric(sf::st_coordinates(.)[,2])) |>
  sf::st_set_geometry(NULL) |>
  #dplyr::select(-featurecla) |>
